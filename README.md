...
- **版本发布**：上传安装包、SHA-256 校验、Ed25519 签名、按设备稳定分桶灰度放量。
- **远程更新**：客户端自动检查、验签、下载、切换版本，四种语言 SDK 行为一致（见 [接入指南 7.6](docs/INTEGRATION.md)）。
- **分发页**：给每个应用生成对外下载页 `/d/<链接名>`，可要求验卡后才给下载（见下）。
...
给新用户下载安装包的对外页面，一个应用一个：**应用管理 → 分发页**。

开启后得到一个链接 `https://你的域名/d/<链接名>`，页面上可配置标题、简介、Logo、
使用说明（支持极简 markdown）、更新日志、客服方式和购卡入口。关闭开关链接立刻 404。

**要理解的一件事：更新包和安装包是两个文件。**

| 包 | 给谁 | 在哪上传 |
|----|------|----------|
| 更新包 | 已经装了旧版的用户，由 SDK 自动下载安装 | 发布版本时的「安装包」 |
| 完整安装包 | 新用户，从分发页手动下载 | 发布版本时的「完整安装包」 |

以 PyInstaller 项目为例：更新包里只有主程序 exe，完整安装包还要带上启动器和运行时。
只传更新包的话，新用户下载下来会发现双击没反应——因为缺了启动器。

**验卡下载**：开启「需要卡密」后，用户要先输入有效卡密才能拿到下载地址。
验卡只判断卡密本身可用，**不绑定设备、不消耗名额**（这时还没装上，占额度等于让用户白白损失一个）。
通过后发一张 10 分钟有效的下载票据，转发出去很快就失效。

页面对所有后台配置的文本做转义后再渲染，正文里写 HTML 只会显示成文字。

> 部署提示：`/d/` 路径需要反代到后端。仓库里的 `packages/admin/nginx.conf` 已经配好，
> 用自己的反代时记得加上，否则会被 SPA 兜底成后台首页。
